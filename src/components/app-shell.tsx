...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-mesh flex">
      ...
      <aside className="hidden md:flex w-60 fixed inset-y-0 left-0 z-40 glass flex-col">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] font-medium transition-all duration-200 ${active ? "bg-emerald-50 text-emerald-700" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
              >
        ...
        <div className="p-3 border-t border-slate-200/60">
          <div className="flex items-center gap-2.5 mb-2.5 px-1">
            ...
            {user?.role === "admin" && <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">Admin</span>}
          </div>
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 glass border-t border-slate-200/60 flex overflow-x-auto py-2 z-50">
        {navItems.map((item) => {
          const Icon = item.icon;
          const active = page === item.page;
          return (
            <button key={item.page} onClick={() => navigate(item.page)} className={`flex flex-col items-center text-[10px] px-3 whitespace-nowrap transition-colors ${active ? "text-emerald-600" : "text-slate-400"}`}>
