...
interface Report { id: string; report_type: string; generated_at: string; }
...
function ReportsContent() {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div><h1 className="text-2xl font-bold text-slate-900 tracking-tight">Reports</h1><p className="text-sm text-slate-400 mt-0.5">Generate downloadable PDF reports for your records.</p></div>
      <div className="grid md:grid-cols-3 gap-4">
        {reportTypes.map((r, i) => {
          const Icon = r.icon;
          const last = lastGenerated(r.type);
          return (
            <div key={r.type} className="glass rounded-2xl p-5 card-hover animate-slide-up" style={{ animationDelay: `${i * 60}ms` }}>
              ...
              <Button size="sm" onClick={() => handleGenerate(r.type)} disabled={generating === r.type} className="bg-emerald-500 hover:bg-emerald-600 w-full">
                {generating === r.type ? <><Loader2 className="h-3.5 w-3.5 animate-spin mr-1" />Generating PDF...</> : <><Download className="h-3.5 w-3.5 mr-1" />Generate PDF</>}
              </Button>
            </div>
          );
        })}
