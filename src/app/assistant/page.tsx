...
export default function AssistantPage() {
  return <ProtectedRoute><AppLayout><AssistantContent /></AppLayout></ProtectedRoute>;
}
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="glass rounded-2xl flex flex-col h-[calc(100vh-280px)] min-h-[400px]">
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {messages.length === 0 && <div className="text-center py-12"><div className="h-12 w-12 rounded-xl bg-emerald-50 flex items-center justify-center mx-auto mb-3"><MessageSquare className="h-6 w-6 text-emerald-500" /></div><p className="text-sm text-slate-500">Ask me anything about your finances.</p><p className="text-xs text-slate-400 mt-1">I answer only from your verified data — no investment advice.</p></div>}
          {messages.map((m, i) => <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm animate-slide-up ${m.role === "user" ? "bg-emerald-500 text-white rounded-br-sm" : "bg-slate-100 text-slate-700 rounded-bl-sm"}`}>{m.text}</div>
          </div>)}
