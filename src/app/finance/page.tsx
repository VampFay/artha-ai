...
export default function FinancePage() {
  return <ProtectedRoute><AppLayout><FinanceContent /></AppLayout></ProtectedRoute>;
}
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex justify-between items-end">
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Emergency Fund</label>
          <input type="number" value={emergencyFund} onChange={(e) => setEmergencyFund(e.target.value)} className="w-32 h-9 rounded-lg border border-slate-200 bg-white/50 px-3 text-sm outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20" />
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="glass rounded-2xl p-6 animate-slide-up stagger-1">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={ie} margin={{ top: 10, right: 0, bottom: 0, left: -20 }}>
              <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#94a3b8" }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} tickFormatter={(v) => `₹${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} />
              <Tooltip formatter={(v) => formatINR(Number(v))} contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 12 }} />
        ...
        <div className="glass rounded-2xl p-6 animate-slide-up stagger-2">
          ...
          {cats.length === 0 ? <p className="text-sm text-slate-400 py-16 text-center">No expense data.</p> : (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={cats} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={75} innerRadius={40}>{cats.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}</Pie>
                <Tooltip formatter={(v) => formatINR(Number(v))} contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 12 }} />
      ...
      {data.suggestions.length > 0 && (
        <div className="glass rounded-2xl p-5 animate-slide-up stagger-3 border-l-4 border-emerald-400">
          <span className="text-xs font-medium text-emerald-600 uppercase tracking-wider">Suggestions</span>
          <div className="mt-2 space-y-2">{data.suggestions.map((s, i) => <p key={i} className="text-sm text-slate-700">• {s}</p>)}</div>
        </div>
      )}
    </div>
  );
