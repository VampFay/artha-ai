...
export default function SettingsContent() {
  ...
  const load = () => { Promise.all([consent.history(), audit.list()]).then(([c, a]) => { setConsents(c.items); setLogs(a.items); setError(""); }).catch(() => setError("Failed")).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, []);
  const handleExport = async () => { try { const token = localStorage.getItem("finsight_token"); const res = await fetch("/api/users/me/export", { headers: { Authorization: `Bearer ${token}` } }); if (!res.ok) throw new Error("Export failed"); const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `finsight_export.json`; a.click(); URL.revokeObjectURL(url); toast({ title: "Data exported!" }); } catch (e: any) { toast({ title: "Export failed", variant: "destructive" }); } };
  const handleRevoke = async () => { if (!confirm("Revoke consent? You won't be able to upload documents.")) return; setRevoking(true); try { const token = localStorage.getItem("finsight_token"); await fetch("/api/consent/revoke", { method: "POST", headers: { Authorization: `Bearer ${token}` } }); toast({ title: "Consent revoked." }); load(); } catch { toast({ title: "Failed", variant: "destructive" }); } finally { setRevoking(false); } };
  const handleLogout = async () => { await logout(); navigate("dashboard"); };
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="glass rounded-2xl p-5 animate-slide-up stagger-1">
          <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><Shield className="h-4 w-4 text-emerald-500" /><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Consent</span></div>{hasActiveConsent && <Button size="sm" variant="outline" onClick={handleRevoke} disabled={revoking} className="text-red-500 border-red-200 hover:bg-red-50"><ShieldX className="h-3 w-3 mr-1" />{revoking ? "..." : "Revoke"}</Button>}</div>
          {consents.length === 0 ? <p className="text-sm text-slate-400">No consent events.</p> : <div className="space-y-2">{consents.map((c) => <div key={c.id} className="flex justify-between text-sm py-1.5 border-b border-slate-100 last:border-0"><span className="text-slate-600">{c.consent_type}</span><div className="text-right"><p className="text-xs text-slate-500">{formatDateTime(c.accepted_at)}</p>{c.revoked_at && <p className="text-xs text-red-400">Revoked</p>}</div></div>)}</div>}
        </div>
      ...
      <div className="glass rounded-2xl p-5 animate-slide-up stagger-3">
        ...
        <div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={handleExport}><Download className="h-3.5 w-3.5 mr-1" />Export Data</Button><Button variant="outline" size="sm" onClick={handleLogout}><LogOut className="h-3.5 w-3.5 mr-1" />Logout</Button></div>
      </div>
    </div>
  );
}
