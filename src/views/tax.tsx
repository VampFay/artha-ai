    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="glass rounded-2xl p-5 animate-slide-up stagger-2">
          ...
          <div className="mt-3 space-y-2 text-sm">{Object.entries(data.income_summary).map(([k, v]) => <div key={k} className="flex justify-between"><span className="text-slate-500 capitalize">{k}</span><span className="font-medium text-slate-900">{formatINR(v)}</span></div>)}</div>
        </div>
