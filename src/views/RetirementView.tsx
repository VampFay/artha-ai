...
export default function RetirementView() {
  ...
  // Very rough approximation of required monthly SIP
  const requiredSIP = (targetCorpus - currentCorpus * Math.pow(1 + returnRate, yearsToRetire)) / 
    ...
    <div className="flex flex-col px-6 lg:px-12 max-w-[1200px] mx-auto w-full">
      <div className="pt-8 pb-6 border-b border-carbon/10 mb-12">
        ...
        <p className="text-sm text-stone max-w-2xl">
          Simulate your path to Financial Independence and Early Retirement (FIRE) based on your current corpus, target age, and estimated lifestyle costs.
        </p>
      </div>
          ...
          <div className="bg-[#FAFAFA] p-8 border border-carbon/5 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <input 
                ...
                className="w-full accent-carbon"
              />
            </div>
            ...
            <div className="space-y-4">
              <label className="block text-xs font-medium text-stone">Today's Monthly Expense (₹{monthlyExpense.toLocaleString()})</label>
              <input 
                type="range" 
            ...
            <div className="space-y-4">
              ...
              <input 
                ...
                value={currentCorpus} 
                onChange={(e) => setCurrentCorpus(parseInt(e.target.value))}
                className="w-full accent-carbon"
              />
            </div>
        ...
        >
          ...
          <div>
            ...
            <div className="h-64 flex items-end gap-1 relative border-b border-l border-carbon/20 pb-2 pl-2">
              ...
              {/* Fake bars indicating growth */}
              {Array.from({ length: yearsToRetire + 1 }).map((_, i) => {
                const year = currentAge + i;
                const value = currentCorpus * Math.pow(1 + returnRate, i) + 
                              requiredSIP * (((Math.pow(1 + returnRate/12, i * 12) - 1) / (returnRate/12)) * (1 + returnRate/12));
                const heightPercentage = Math.min(100, (value / targetCorpus) * 100);
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group">
                    <motion.div 
                      ...
                      className={cn("w-full max-w-[12px] rounded-t-sm transition-colors", 
                        i === yearsToRetire ? "bg-saffron" : "bg-carbon/80 group-hover:bg-carbon"
                      )}
                    />
                  </div>
                );
