            ...
            return (
              <motion.div key={g.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="p-8 border border-carbon/10 bg-[#FAFAFA]">
                ...
                <div className="grid grid-cols-3 gap-4 pt-4 border-t border-carbon/5">
                  <div><span className="block text-[10px] text-stone uppercase tracking-wider mb-1">Saved</span><span className="font-geist-pixel text-sm text-carbon">₹{g.currentAmount.toLocaleString("en-IN")}</span></div>
                  <div><span className="block text-[10px] text-stone uppercase tracking-wider mb-1">Monthly</span><span className="font-geist-pixel text-sm text-carbon">₹{g.monthlyContribution.toLocaleString("en-IN")}</span></div>
                  <div><span className="block text-[10px] text-stone uppercase tracking-wider mb-1">ETA</span><span className="font-geist-pixel text-sm text-carbon">{g.projection?.projected_completion_date ? new Date(g.projection.projected_completion_date).toLocaleDateString("en-IN", { year: "numeric", month: "short" }) : "—"}</span></div>
                </div>
