...
export default function LiabilitiesView() {
  ...
  const { data, isLoading: loading } = useLiabilities();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div key={loan.id} className="p-6 md:p-8 border border-carbon/10 bg-[#FAFAFA] group hover:border-carbon/30 transition-all">
                  <div className="flex justify-between items-start mb-8">
                    ...
                    <div className="text-right">
                      <div className="text-lg font-geist-pixel text-carbon">₹{loan.remaining.toLocaleString("en-IN")}</div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="space-y-6">
          <div className="p-8 bg-carbon text-white">
            ...
            <div className="text-3xl font-light mb-1"><span className="font-michroma text-stone mr-1">₹</span><KineticNumber value={totalDebt} /></div>
            <p className="text-xs text-stone mt-4 mb-6">Total Monthly EMI outflow: <span className="text-white font-geist-pixel">₹{totalEmi.toLocaleString("en-IN")}</span></p>
          ...
          {data?.prepaymentInsight && (
            <div className="p-8 border border-saffron/20 bg-saffron/5">
              ...
              <button onClick={() => toast({ title: "Prepayment Insight", description: data?.prepaymentInsight || "Upload loan details to see prepayment insights." })} className="text-xs text-saffron font-bold uppercase tracking-wider hover:text-carbon transition-colors">Simulate Prepayment</button>
            </div>
          )}
        </motion.div>
      </div>
