/**
 * Seed new Artha AI tables (Portfolio, Cashflow, Liabilities, Estate, Subscriptions)
 * with sample data for the test user.
 */
import { db } from "../src/lib/db";

async function main() {
  const user = await db.user.findFirst({ where: { email: "test@finsight.ai" } });
  if (!user) { console.error("Test user not found. Run scripts/seed-accounts.ts first."); process.exit(1); }
  ...
  for (const a of assets) {
    await db.assetHolding.create({ data: { userId, ...a } });
  }
  console.log(`✓ ${assets.length} asset holdings seeded`);
  ...
  for (const t of targets) {
    await db.allocationTarget.create({ data: { userId, ...t } });
  }
  console.log(`✓ ${targets.length} allocation targets seeded`);
  ...
  for (const s of subs) {
    await db.subscription.create({ data: { userId, ...s, status: "Active" } });
  }
  ...
  for (const l of loans) {
    await db.liability.create({ data: { userId, ...l } });
  }
  console.log(`✓ ${loans.length} liabilities seeded`);
  ...
  for (const n of nominees) {
    await db.nominee.create({ data: { userId, ...n } });
  }
  console.log(`✓ ${nominees.length} nominees seeded`);

  console.log("\n✅ All new tables seeded successfully!");
  console.log("Total portfolio value: ₹" + (assets.reduce((s, a) => s + a.value, 0) / 10000000).toFixed(2) + " Cr");
}
